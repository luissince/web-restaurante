...
const NavbarMobile = () => {
  ...
  return (
    ...
    >
      ...
      <div
        className={`lg:hidden w-full mx-auto px-3 pt-2 ${
          menuOpen ? "block" : "hidden"
        }`}
      >
        <ul className="flex flex-col text-lg font-medium text-white">
          <li className="my-3">
            <a className="opacity-50 w-full " href="/">
              Inicio
            </a>
          </li>
          <li className="my-3">
...
function NavbarMultiDevice() {
  ...
  return (
    <nav className={`flex bg-red py-5 items-center flex-nowrap justify-start fixed top-0 left-0 w-full z-50 ${backgroundColor}`} arial-label="Furni navigation bar" >
        <div className="h-24 z-50 relative container mx-auto px-0 lg:px-6 grid grid-cols-3">
            <div className="flex items-center">
                ...
                <div className={`absolute transition-all duration-700 right-0 ${ menuOpen ? "w-full block absolute ml-auto mr-auto top-[250%] left-0 right-0 text-center" : "text-center -top-[700px] -left-[2000px] mx-auto"}`}>
                    ...
                    <Link className="text-4xl block mt-5 text-white" href="/ordenar" onClick={toggleMenu}>
                        <span className={`${pathName === "/ordenar" ? "border-b-4 border-b-secondary" : "opacity-50" }`}>Pide Ahora</span>
                    </Link>
                    ...
                    <Link className="text-4xl block mt-5 text-white" href="/locales" onClick={toggleMenu}>
                        <span className={`${pathName === "/locales" ? "border-b-4 border-b-secondary" : "opacity-50" }`}>Locales</span>
                    </Link>
                </div>
            </div>

            <div className="flex items-center justify-center ">
                <a href="/" className="text-white uppercase font-bold text-base lg:text-2xl tracking-widest">Rapido y Sabroso</a>
            </div>
